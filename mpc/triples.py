...
class TripleGeneration:
	...
	def __init__(self, index, shamir, dispatcher, batch_size=1000, n_batches=1):
	...
	def run(self):
		for k in range(self.n_batches):
			...
			for i in range(len(msgs)):
				if i+1 != self.index:
					self.dispatcher.send(i+1, "TRIP-AB", round_, msgs[i])
			resps = self.dispatcher.collect(round_, full_quorum=True)
			...
			for i in range(len(msgs)):
				if i+1 != self.index:
					self.dispatcher.send(i+1, "TRIP-C", round_, msgs[i])
			resps = self.dispatcher.collect(round_, full_quorum=True)
			resps.append(msgs[self.index-1])

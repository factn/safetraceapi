...
if __name__ == '__main__':
	...
	q = Queue()
	procs = [Process(target=run_client_op, args=(clients[i], 300+i, ''.join([random.choice([i for i in 'abcdefghijklmnopqrstuvwxyz123456789'])]), q)) for i in range(2)]
	for p in procs:
		p.start()
	for p in procs:
		p.join()
	...
	procs = [Process(target=run_client_op, args=(clients[i], 5000+(i*100), ''.join([random.choice([i for i in 'abcdefghijklmnopqrstuvwxyz123456789'])]), q)) for i in range(2)]
	for p in procs:
		p.start()
	for p in procs:
		p.join()
